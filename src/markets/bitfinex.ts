...
import { EmptyMarketData } from '../models/EmptyMarketData'
...
function formatData (data: [[string | number]]): MarketData {
  ...
  data[0].forEach(ticker => {
    ...
    const bids = getBids(ticker)
    const asks = getAsks(ticker)
    formattedData[name] = { asks, bids }
  })
  return { [this.marketName]: formattedData }
}

function getBids (ticker: string | number): [[number]] {
  return [[ticker[1]]]
}

function getAsks (ticker: string | number): [[number]] {
  return [[ticker[3]]]
}

function getName (ticker): string {
  return ticker[0].substring(1)
}
