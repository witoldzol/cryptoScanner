...
let transactionCostAdjustment = (
  currencyRate: number,
  MARKET_TRANSACTION_RATES: object,
  market: string,
  transaction: string
): number => {
  if (transaction === "ask") {
...
let getAsksFirstElement = (data: AsksBids): number[] =>
  calculateAskRate(data["asks"][0]);
...
let calculateEdgeValues = (
  ...
  let adjustedRateForMarketFee: number = transactionCostAdjustment(
    arrWithPriceAndVolume[0],
    MARKET_TRANSACTION_RATES,
    marketName,
    askOrBid
  );
...
let createEdges = (
  ...
  graph.addEdge(
    pairOne(pair),
    pairTwo(pair),
    calculateEdgeValues(getBidsFirstElement(values), marketName, "bid")
  );
  //ASK - we buy
  graph.addEdge(
    pairTwo(pair),
    pairOne(pair),
    calculateEdgeValues(getAsksFirstElement(values), marketName, "ask")
  );
};
